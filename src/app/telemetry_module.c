...
#define MAX_MSG_LEN 18  /*  all messages should be 32 char long (including \r\n, so 30 total
...
/*F************************************************************************************************
 * NAME: void Telemetry_Module_Notify(MessageType messageType, MessageSeverity messageSeverity, const char *msg)
 *
 * DESCRIPTION:
 *      Sends a message using bluetooth HAL. A message is about an Event, that must have a type and severity
 *      
 *
 * INPUTS:
 *      PARAMETERS:
...
void Telemetry_Module_Notify(MessageType messageType, MessageSeverity messageSeverity, const char *msg){
    BT_HAL_sendMessage("type:%d" SEPARATOR "sev:%d"  SEPARATOR "%s" SEPARATOR, messageType, messageSeverity, msg);           
}
 ...
 *      [4] sets meta information about the message, namely type and severity depending on battery percentage
...
void Telemetry_Module_SendMsgMotorSpeedChange(Message_MotorSpeedUpdate *speedUpdate){
    ...
    Telemetry_Module_Notify(
        speedUpdate->messageInfo.type,
        speedUpdate->messageInfo.severity,
        buffer
    );
}
 ...
 *
 * DESCRIPTION:
 *      This functions reads data from a motor direction message struct, formats it and puts it into a
 *      string buffer. It then calls Telemetry_Module_Notify to actually send the data over BT
 *
 * INPUTS:
 *      PARAMETERS:
...
void Telemetry_Module_SendMsgMotorDirChange(Message_MotorDirectionUpdate *dirUpdate){
    ...
    Telemetry_Module_Notify(
        dirUpdate->messageInfo.type,
        dirUpdate->messageInfo.severity,
        buffer
    );
}
 ...
 *
 * DESCRIPTION:
 *      This functions reads data from a object detected message struct, formats it and puts it into a
 *      string buffer. It then calls Telemetry_Module_Notify to actually send the data over BT
 *
 * INPUTS:
 *      PARAMETERS:
 ...
 */
void Telemetry_Module_SendMsgObjectDetected(Message_ObjectDetected *objectDetected){
    sprintf(buffer, "dir:%d" SEPARATOR "dst:%d", objectDetected->direction, objectDetected->distance);

    Telemetry_Module_Notify(
        objectDetected->messageInfo.type,
        objectDetected->messageInfo.severity,
        buffer
    );
}
