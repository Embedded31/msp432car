 ...
 *
 * DESCRIPTION:
 *      Bluetooth Hardware Abstraction Layer (HAL), this source file provides
 *      an abstraction over the UART communications with the Bluetooth Low
 ...
 *
 * GLOBAL DATA:
 *      char    message[BT_BUFFER_SIZE]     Contains the unread message
 *      int     currentRxChar               Indicates the current char to read
 *      bool    isMessageReady              If there is a pending message
 *      char*   currentTxChar               Indicates the current char to send
 *      bool    isMessageSent               If the transmission is terminated
 *
 * NOTES:
 *      [1] Every time that a reception interrupt is generated by the EUSCI
 *          module  related to the Bluetooth the IRQHandler provided in this
 *          file will read the incoming message and store it in a global
 *          variable, then it will set the messageReady global variable to 1.
 *      [2] The global variables are declared as static volatile due to
 *          the interaction with the IRQHandler to void compiling issues.
 *
...
static volatile char message[BT_BUFFER_SIZE];
static volatile int currentRxChar = 0;
static volatile bool isMessageReady = false;
static volatile const char *currentTxChar;
static volatile bool isMessageSent = true;
 ...
 *
 * DESCRIPTION:
 *      Sends a message to the BLE module via the UART communication,
 *      the bluetooth module will forward it and every connected device will
 *      receive it, the procedure goes through the following steps:
 ...
 *
 * DESCRIPTION:
 *      Copies the unread message to the buffer passed as parameter, if there
 *      isn't an unread message the empty string "\0" is copied.
 ...
 *      GLOBALS:
 *          bool        isMessageReady      Set to false after
 *
 *  NOTE:
 *      Only the first min(BT_BUFFER_SIZE, length) characters will be copied
 ...
 *
 * DESCRIPTION:
 *      This function is called every time that an interrupt regarding the
 *      Bluetooth EUSCI module raises, two procedures are executed:
 *      RECEIVE_INTERRUPT:  the interrupt signals that there is a character in
 *                          the RX buffer, read it, store it in the message
 *                          buffer and update the currentRxChar; if the end of
 *                          string char is read ('\n', '\r' or '\0') set the
 *                          isMessageReady variable to true.
 *      TRANSMIT_INTERRUPT: the interrupt signals that the TX buffer is ready,
 *                          the currentTxChar is sent and the pointer is
 *                          updated to the next char; if the currentChar is
 *                          equal to '\0' then set the isMessageSent to true
 *                          and disable the Transmit interrupts.
 *
 * INPUTS:
 *      PARAMETERS:
 *          None
 ...
 *          const char*     currentTxChar       Current char to send
 *
 *  OUTPUTS:
 *      PARAMETERS:
 *          None
 ...
 *
 *  NOTE:
 *      If the reception interrupt is triggered while there is an unread
 *      message this one will be lost and overwrite with the new one
 */
...
void EUSCIA2_IRQHandler(void) {
    ...
    if (status & EUSCI_A_UART_RECEIVE_INTERRUPT_FLAG) {
        ...
        /* if there is an unread message and we have other terminators
         * on the buffer just skip these characters, this happens when
         * the strings are terminated with "\r\n" */
        if (isMessageReady && (r == '\n' || r == '\r' || r == '\0')) {
            return;
        }

        /* if new characters are on the buffer overwrite the current
         * stored message */
        if (isMessageReady) {
            isMessageReady = false;
            currentRxChar = 0;
    ...
    if (status & EUSCI_A_UART_TRANSMIT_INTERRUPT_FLAG) {
        ...
        if (*currentTxChar) {
            ...
            UART_disableInterrupt(BT_EUSCI_BASE,
                                  EUSCI_A_UART_TRANSMIT_INTERRUPT);
            UART_clearInterruptFlag(BT_EUSCI_BASE,
